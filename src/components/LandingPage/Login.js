        ...
        <>
                <form onSubmit={handleSubmit} className="flex flex-col items-center justify-start w-full bg-white">
                    ...
                    {error && (
                        <div className="flex items-center w-full px-6 py-4 mx-2 my-4 text-lg bg-red-200 rounded-md xl:w-2/4">
                            ...
                            <span className="text-red-800">You have entered an invalid username or password.</span>
                        </div>
                        )}
                ...
                <div className="flex items-center w-full text-center mt-7">
                    ...
                    <div className="flex justify-center w-full mt-7">
                        <button onClick={handleFacebookSignIn} className="px-8 py-4 mr-5 text-xl text-white transition duration-500 ease-in-out transform border-none shadow-xl cursor-pointer bg-blue-darkest rounded-xl hover:shadow-inner hover:-translate-x hover:scale-105">
                            Facebook
                        </button>
                        <button  onClick={handleGoogleSignIn} className="px-8 py-4 text-xl text-white transition duration-500 ease-in-out transform bg-red-500 border-none shadow-xl cursor-pointer rounded-xl hover:shadow-inner hover:-translate-x hover:scale-105">
                                Google
                        </button>
                    </div>
                <p className="inline-block w-full py-4 font-semibold text-center text-black text-md">Don't have an account? <Link to="/register" className="text-blue-darkest">Register</Link></p>
            </>
    ) 
}
